...
class Rover {
	...
	land() {
		...
		hasObstacle
			? console.log(
					"do not land foo fast, there's obstacles at coordinates:" +
						JSON.stringify(obstacles)
			  )
			: console.log("Aucun obstacle sur la planete");
	}
	...
	canMove(direction: Ycoordinates): boolean {
		...
		if (hasObstacle) {
			console.log(
				"Il y a un obstacle à la prochaine position. Le rover ne peut pas se déplacer en:" +
	...
	private calculateNewPosition(direction: Ycoordinates): Position {
		const movementMap: Record<Cardinals, Position> = {
			...
			[Cardinals.Ouest]: {
				x: this._position.x - direction,
				y: this._position.y,
			},
		};

		const newPosition: Position =
			movementMap[this._orientation.getOrientation()];
		return {
